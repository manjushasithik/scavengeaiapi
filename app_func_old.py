...
################################################################################################################## Object #############################################################################################
        # faultdict = {"3": '1',"12": '2',"17": '3',"10": '4', "11":'5'}
        ...
        # df["class"] = list(map(lambda x: "1" if x==3 else ("2" if x ==12 else ("3" if x==17 else ("4" if x==10 else "5")) ),df['class'])) #Map classes to class column
...
############################################################### PDF ##################################################
    ...
    doc.tables[0].cell(4, 4).text = "Licence/Builder:"
    ...
    doc.tables[0].cell(7, 1).text = data_pdf_para['Maker']
    ...
    doc.tables[0].cell(7, 3).text = data_pdf_para['Model']
    ...
    doc.tables[1].cell(2, 1).paragraphs[0].runs[0].font.size = Pt(7)
    ...
    doc.tables[1].cell(2, 3).paragraphs[0].runs[0].font.size = Pt(7)
    ...
    doc.tables[1].cell(3,1).paragraphs[0].runs[0].font.size = Pt(7)
    ...
    doc.tables[1].cell(4, 3).paragraphs[0].runs[0].font.size = Pt(7)
    ...
    for i,k in zip(range(len(selection_deposits)),data_k):
        row_cells = doc.tables[3].add_row().cells
        ind=0
        row_cells[0].text = str(k[-1])
    ...
    for i,k in zip(range(len(selection_brekage)),data_k):
...
# Surface section --tables[7]
    ...
    for i,k in zip(range(len(selection_surface)),data_k):
...
# Lubrication section --tables[8]
    ...
    for i,k in zip(range(len(selection_lubrication)),data_k):
...
########### adding images #########################
    ...
    data_pdf={'PDF':"https://scavaivisionapi.azurewebsites.net/files/report.pdf"} 
